...
--
--  Fixes:
--    1. execute_purchase: enforce auth.uid() = p_user_id
--    2. execute_purchase: enforce tier-based entry limits
--    3. execute_purchase: enforce event min/max entry
--    4. execute_purchase: enforce p_gross > 0 and max sanity cap
--    5. execute_sell: enforce auth.uid() = p_user_id
--    6. deposit_balance: add max deposit cap
--    7. withdraw_balance: add max withdrawal cap
--    8. cast_vote: enforce event status + entry limits
--    9. Rate limiting helper for app-level use
-- ============================================================


-- ── 1. execute_purchase — hardened ──────────────────────────────
--
--  NEW checks:
--    ✓ auth.uid() must match p_user_id (prevents impersonation)
--    ✓ p_gross >= event.min_entry AND p_gross <= event.max_entry
--    ✓ user tier >= event.tier_required
--    ✓ p_gross >= 1 (absolute minimum)
--    ✓ p_gross <= per-tier cap (Q500 / Q2000 / Q10000)
...
BEGIN
  ...
  -- ▸ EVENT: check exists and is open
  ...
  -- ▸ ENTRY LIMITS: from event config
  ...
  IF p_gross > v_tier_cap THEN
    RETURN jsonb_build_object('error',
      'Límite para Nivel ' || v_profile.tier || ': Q' || v_tier_cap || '. Subí de nivel para participar más.');
  END IF;
...
-- ── 2. execute_sell — hardened ──────────────────────────────────
--
--  NEW: auth.uid() must match p_user_id
...
BEGIN
  ...
  UPDATE event_markets SET
    ...
    no_shares  = CASE WHEN v_pos.side = 'no'
                      ...
                      ELSE no_shares  END,
    pool_total      = GREATEST(pool_total - v_proceeds, 0),
...
-- ── 3. deposit_balance — hardened ───────────────────────────────
--
--  NEW: max Q50,000 per deposit, max Q100,000 total balance
...
BEGIN
  ...
  IF p_amount > 50000 THEN
    RETURN jsonb_build_object('error', 'Máximo Q50,000 por depósito');
  END IF;
...
-- ── 4. withdraw_balance — hardened ──────────────────────────────
--
--  NEW: max Q25,000 per withdrawal
...
BEGIN
  ...
  IF p_amount > 25000 THEN
    RETURN jsonb_build_object('error', 'Máximo Q25,000 por retiro');
  END IF;
...
--
--  Simple token-bucket style: track recent actions per user.
--  Checked from RPCs or via a helper function.

CREATE TABLE IF NOT EXISTS public.rate_limits (
  id         UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id    UUID          NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  action     TEXT          NOT NULL,  -- 'purchase', 'sell', 'deposit', 'withdraw'
...
-- Helper: check if user exceeded rate limit
CREATE OR REPLACE FUNCTION public.check_rate_limit(
  p_user_id  UUID,
  p_action   TEXT,
  p_max      INTEGER DEFAULT 10,     -- max actions
  p_window   INTERVAL DEFAULT '1 minute'  -- time window
)
RETURNS BOOLEAN
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
...
-- Helper: record an action for rate limiting
...
BEGIN
  ...
  -- Cleanup old entries (older than 1 hour) — keep table small
...
-- ── 6. Add RLS policies for positions (read own + admin) ────────
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'public' AND c.relname = 'positions' AND c.relrowsecurity = true
  ) THEN
    ALTER TABLE public.positions ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

-- Users read own positions
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'positions' AND policyname = 'positions_own_read'
  ) THEN
    CREATE POLICY "positions_own_read"
      ON public.positions FOR SELECT
      USING (auth.uid() = user_id);
  END IF;
END $$;

-- Admin reads all positions
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'positions' AND policyname = 'positions_admin_read'
  ) THEN
...
-- ── 7. Add RLS for market_transactions ──────────────────────────

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'public' AND c.relname = 'market_transactions' AND c.relrowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'market_transactions' AND policyname = 'tx_own_read'
  ) THEN
    CREATE POLICY "tx_own_read"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'market_transactions' AND policyname = 'tx_admin_read'
  ) THEN
...
-- ── 8. Admin-only profiles read (for user management) ──────────

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'profiles_admin_read'
  ) THEN
