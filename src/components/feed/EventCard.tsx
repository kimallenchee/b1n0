...
export function EventCard({ event }: EventCardProps) {
  ...
  return (
    <>
      <div data-tour="event-card" className={`event-card${event.isLive && !isResolved ? ' is-live' : ''}`} style={{ borderLeft: `2px solid ${color}`, overflow: 'hidden', background: 'var(--color-surface)' }}>
        {photo ? (
